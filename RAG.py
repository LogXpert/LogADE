...
class RAGPostProcessor:
    ...
    def post_process(self, anomaly_logs_path, test_data_path):
        ...
        try:
            ...
            self.logger.info(f"Test data columns: {list(test_df.columns)}")
        except Exception as e:
        ...
        if 'LineId' in filtered_df.columns:
            return filtered_df['LineId'].tolist()
        elif 'LineId' in test_df.columns:
            return test_df[test_df['EventTemplate'].isin(anomaly_templates)]['LineId'].tolist()
        else:
            self.logger.warning("No LineId column found, returning row indices")
