    ...
    def train(self, train_loader):
        ...
        for batch in train_loader:
            ...
            if isinstance(model_output, tuple):
                # Multi-head DiGCN returns (emb_list, score_list)
                train_embeddings, _ = model_output
            else:
                # Simple models return emb_list only
                train_embeddings = model_output
            ...
            mean_train_embeddings = [torch.mean(emb, dim=0) for emb in train_embeddings] # Mean-aggregation: G_emb = mean(v_emb for v in G)
    ...
    def test(self, test_loader):
        ...
        with torch.no_grad():
            ...
            for batch in test_loader:
                ...
                if isinstance(model_output, tuple):
                    # Multi-head DiGCN returns (emb_list, score_list)
                    test_embeddings, _ = model_output
                else:
                    # Simple models return emb_list only
                    test_embeddings = model_output
...
class DiGCN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nlayer=2, num_heads=4, dropout=0, bias=False, **kwargs):
